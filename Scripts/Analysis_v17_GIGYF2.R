...
#poorSamples <- readLines("../MIP/txt/files/PiARK2_bad.csv")
#poorSamples <- c("S33024")
#poorSamples <- paste0('MIP.',poorSamples,'.clean.bam',sep='')

good_gene <- paste(readLines("../MIP/txt/cov/good_gene_above_80.cov"),collapse="|")
...
ctrl.dafr <- lapply(ctrl.tmp1, function(x) ifelse(length(which(colMeans(x[,-c(1:6)]) < 50)) > 0,return(x[ , -(as.vector(which(colMeans(x[,-c(1:6)]) < 50))+6)]),return(x)))
...
############### Now call the CNVs
    ...
    saveRDS(result,glue('raw/{pd_number[n]}_result_test_v17.rds'))
}
...
################ Now call the CNVs
    ...
    saveRDS(result,glue('raw/{ctrl_number[n]}_result_test_v17.rds'))
}
