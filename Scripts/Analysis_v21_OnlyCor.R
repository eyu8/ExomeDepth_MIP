...
### Create the aggregate reference set for this sample
                           ...
                           result <- data.frame(Sample = colnames(pd_count.dafr)[i+6],
                                                correlation = cor(all.exons@test, all.exons@reference),
                                                numRef = length(my.choice$reference.choice),
                                                ReferenceSet = paste(my.choice$reference.choice,collapse = ','),
                                                stringsAsFactors = FALSE)
                           
                           return(result)
                       }, mc.cores=ncore)
    saveRDS(result,glue('raw/{pd_number[n]}_Cor_v21.rds'))
}
...
#### Create the aggregate reference set for this sample
                           ...
                           all.exons <- new('ExomeDepth',
                                            ...
                                            data = candidate_control.dafr,
                                            formula = 'cbind(test, reference) ~ GC')
                           result <- data.frame(Sample = colnames(candidate_control.dafr)[i+6],
                                                correlation = cor(all.exons@test, all.exons@reference),
                                                numRef = length(my.choice$reference.choice),
                                                ReferenceSet = paste(my.choice$reference.choice,collapse = ','),
                                                stringsAsFactors = FALSE)
    ...
    saveRDS(result,glue('raw/{ctrl_number[n]}_Cor_v21.rds'))
}
