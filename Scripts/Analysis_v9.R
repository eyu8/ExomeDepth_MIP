...
duplicates <- readLines("../MIP/txt/data/NeuroX_duplicate_v2.data")
duplicates <- paste0('MIP.',duplicates,'.clean.bam',sep='')

poorSamples <- readLines("../MIP/txt/files/GBA_control_with_CNV.files")
#poorSamples <- c("S33024")
#poorSamples <- paste0('MIP.',poorSamples,'.clean.bam',sep='')

good_gene <- paste(readLines("../MIP/txt/cov/good_gene_above_90_and_gba.cov"),collapse="|")
...
pd.dafr <- lapply(pd.tmp1, function(x) ifelse(length(which(colMeans(x[,-c(1:6)]) < 50)) > 0,return(x[ , -(as.vector(which(colMeans(x[,-c(1:6)]) < 50))+6)]),return(x)))
...
ctrl.tmp2 <- lapply(ctrl.tmp1, function(x) ifelse(length(which(colMeans(x[,-c(1:6)]) < 50)) > 0,return(x[ , -(as.vector(which(colMeans(x[,-c(1:6)]) < 50))+6)]),return(x)))
...
control.list.mat <- lapply(ctrl.tmp2, function(x) as.matrix(x[, grep(names(x), pattern = '*.clean.bam')]))
...
############### Now call the CNVs
    ...
    saveRDS(result,glue('raw/{pd_number[n]}_result_test_v9.rds'))
}
...
################ Now call the CNVs
    ...
    saveRDS(result,glue('raw/{ctrl_number[n]}_result_test_v9.rds'))
}
