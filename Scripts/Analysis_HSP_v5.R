...
saveRDS(result,"raw/HSP_result_v5_all.rds")


nsamples <- ncol(ctrl_wCNV.mat)
result <- mclapply(1:nsamples,
                   function(i){
                           #### Create the aggregate reference set for this sample
                       my.choice <- select.reference.set (test.counts = ctrl_wCNV.mat[,i],                                                                                                        
                                                          reference.counts = ctrl_wCNV.mat[,-i],
                                                          data = ctrl,
                                                          formula = 'cbind(test, reference) ~ GC')
                       my.reference.selected <- apply(X = ctrl_wCNV.mat[, my.choice$reference.choice, drop = FALSE],
                                                      MAR = 1,
                                                      FUN = sum)
                       message('Now creating the ExomeDepth object')
                       all.exons <- new('ExomeDepth',
                                            test = ctrl_wCNV.mat[,i],
                                            reference = my.reference.selected,
                                            data = ctrl,
                                            formula = 'cbind(test, reference) ~ GC')
                       ################ Now call the CNVs
...
saveRDS(result,'raw/HSP_control_result_v5_all.rds')
